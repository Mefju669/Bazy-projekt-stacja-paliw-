...
public class OptionsWindow implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == zarzadzajTowaremButton) {
            new OpcjeProduktowWindow();
        }
        if(e.getSource() == zarzadzajPracownikamiButton){
            new OpcjePracownikowWindow();
        }
        if(e.getSource() == zarzadzajKategoriamiProduktowButton){
            new OpcjeKategoriiWindow();
        }
        if(e.getSource() == generujRaportyButton){
            new OpcjeRaportyWindow();
        }
        if(e.getSource() == wylogujButton){
            window.dispose();
            new LoginWindow();
        }
        if(e.getSource() == zakonczProgramButton){
            window.dispose();
        }
